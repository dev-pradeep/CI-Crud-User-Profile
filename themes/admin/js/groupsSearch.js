$(document).ready(function() {
	...
	$('#selectGroupsPage').searchableOptionList();
	$('#main_drop_val_select').searchableOptionList();
	
    $('body').on('click','.sol-label-text',function(){
        var checkboxValue =$("input[type='checkbox']").val();
        if(checkboxValue.length > 0) {
            $('#selectError').hide();
        }
    });
	$('.checkbox_list').prop("disabled", true);
	$('#search_bar').prop("disabled", false);
	$('input[type=radio][name=radio]').change(function() {
		if (this.value == 'searchUser') {
			$('.checkbox_list').prop("disabled", true);
			$('.checkbox_list').prop("checked", false);
			$('#search_bar').prop("disabled", false);
		}
		if (this.value == 'existingType') {
			$('.checkbox_list').prop("disabled", false);
			$('#search_bar').prop("disabled", true);
		}
	});
    //handle click of create group
    $('#All').click(function(){
	...
	$('input[type=radio][name=radio1]').change(function() {
		if (this.value == 'edit_searchUser') {
			$('.sol-selection').attr("style", "display:block;");
			$('.edit_check_list').prop("disabled", true);
			$('.edit_check_list').prop("checked", false);
			setTimeout(function(){
				$('.search_box').prop("disabled", false);	
			},100)
		}
		if (this.value == 'edit_existingType') {
			$('.sol-selection').attr("style", "display:none;");
			$('.edit_check_list').prop("disabled", false);
			setTimeout(function(){
	...
	$('#editAll').click(function(){
		$('#editCustomer').prop("checked", false);
		$('#editSeller').prop("checked", false);
		$('#editAdministrator').prop("checked", false);
		$('#editSubscriber').prop("checked", false);
	});
    ...
    $('#create_group').click(function(){
        // var selectedValue = $('#selectGroupsPage').val();
        // if(selectedValue.length > 0) {
            // if($('#groupModalFrm').valid()){
                // create_group();
            // }
        // }else{
            // $('#selectError').show().css('color','red');
        // }
		//console.log($("name="check_list[]"").val());
		create_group();
    });
    //validation for create group
   $('#groupModalFrm').validate({        
        rules: {
        name: {
            required: true  
        },
    ...
    messages : {
        name:{
                required : 'Group Name filed is required',
            },

        },
    });
   //handle click of Edit group icon.
	$("body").on("click","a.editGroups",function(){
		//$("#sol_selection").html('');
		$("#tableGroupCard").hide();
		$("#formGroupCardAdd").hide();
		$("#formGroupCardEdit").show();
		var group_Data = $(this).data('group');
		var groupData = JSON.parse(window.atob(group_Data));
		$("#EditgroupName").val(groupData.group_name);	
		$("#group_id").val(groupData.group_id);	
		var record = groupData.group_record;
		var res = record.split(",");
		var existing_type_user = groupData.existing_type_user;
		var existing_type_user_res = existing_type_user.split(",");
		console.log(existing_type_user_res);
		//
		var elems = document.querySelectorAll('#main_drop_val_select option');
		var values = Array.prototype.map.call(elems, function(obj) {
			var item=obj.value
			if(res.indexOf(item) >= 0){
				var recordData = { "type": "option", "value": item, "label": item, "selected": true};
			}else{
				var recordData = { "type": "option", "value": item, "label": item, "selected": false};  
			}
			return recordData;
		});
		$('#setHtml').html(`<select id="selectGroupsPageEdit"  name="group_character_edit[]" multiple="multiple"></select>`);
		$('#selectGroupsPageEdit').searchableOptionList({
			data: values
		});
		//
		$('.edit_check_list').prop("checked",false);
		if(record == ''){
			console.log("re");
			$('#search_bar').prop("disabled", true);
			$("#existingTypeEdit").prop("checked", true);			
			for(var i=0;i< existing_type_user_res.length;i++){				
				$('.edit_check_list[value="'+existing_type_user_res[i]+'"]').prop("checked", true);
			}
			setTimeout(function(){
				$('.search_box').prop("disabled", true);	
			},100)			
		...
		if(existing_type_user == ''){			
			$('#search_bar').prop("disabled", false);
			$("#searchUserEdit").prop("checked", true);
			setTimeout(function(){
				$('.search_box').prop("disabled", false);	
			},100)
		}
    ...
    //handle click of update btn.
	$('#edit_group').click(function(){
		update_group();
	});
    //handle click of newGroupBtn.
	$('#newGroupBtn').click(function(){
		$("#tableGroupCard").hide();
		$("#formGroupCardEdit").hide();
		$("#formGroupCardAdd").show();
		$('#groupModalFrm')[0].reset();
		$('.checkbox_list').prop("disabled", true);
		$('#search_bar').prop("disabled", false);
	});
    //handle click of deleteGroups icon.
	$("body").on("click","a.deleteGroups",function(){
		var group_Data = $(this).data('group');
		var groupData = JSON.parse(window.atob(group_Data));
		var group_id = groupData.group_id;
		displayDeletegroup(group_id);
	});
    //handle click of cancel to add group.
	$('#cancelAdd').click(function(){
		$("#formGroupCardAdd").hide();
		$("#formGroupCardEdit").hide();
		$("#tableGroupCard").show();
	});
    //handle click of back btn  when add group.
	$('#backListAdd').click(function(){
		$("#formGroupCardAdd").hide();
		$("#formGroupCardEdit").hide();
		$("#tableGroupCard").show();
	});
    //handle click of cancel to edit group.
	$('#cancelEdit').click(function(){
		$("#formGroupCardAdd").hide();
		$("#formGroupCardEdit").hide();
		$("#tableGroupCard").show();
	});
    //handle click of back btn  when update group.
	$('#backListEdit').click(function(){
		$("#formGroupCardAdd").hide();
		$("#formGroupCardEdit").hide();
		$("#tableGroupCard").show();
		$('#groupModalFrmEdit')[0].reset();
	});
});
function create_group(){ // create group
	var d=$('#groupModalFrm').serializeArray();	
	var url=$('#base_url').val();	
	$.ajax({
		type:"post",
		data:d,
		url: url+'admins/create_group',
...
function update_group(){ //update group
	var d=$('#groupModalFrmEdit').serializeArray();	
	var url=$('#base_url').val();	
	$.ajax({
		type:"post",
		data:d,
		url: url+'admins/update_group',
		success: function(data){        	
			if(data.status=="success"){
				toastr.success(data.message);
				//$("#GroupModalEdit").modal('hide');
				$("#formGroupCardAdd").hide();
				$("#formGroupCardEdit").hide();
				$("#tableGroupCard").show();
				$('#groupModalFrmEdit')[0].reset();
...
function displayDeletegroup(id){ // added swal when delete group
	swal({
		title: "Delete",
...
function delete_group(id) { // delete group
	var url = $('#base_url').val();
	$.ajax({
		type: 'post',
		data: {group_id: id},
		url: url + 'admins/delete_group',
		success: function(data) {
			getGroupData();
		}
	});			
}
function getGroupData(){ // getGroupData
	var url=$('#base_url').val();
	$.ajax({
		type:"post",
		data:{},
		url:url +'admins/getGroupData',
		success: function(data){
...
(function ($, window, document) {
    ...
    SearchableOptionList.prototype = {
        ...
        _initializeUiElements: function () {
            ...
            this.$input = $('<input id="search_bar" type="text" class="search_box" />')
        ...
        _addSelectionDisplayItem: function ($changedItem) {
            ...
            if (!$existingDisplayItem) {
                ...
                //$existingDisplayItem = $('<div id="sol_selection" class="sol-selected-display-item"/>')
